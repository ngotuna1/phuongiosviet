...
(function () {
  ...
  function installMenu(){
    ...
    function ensureMenu(){
      ...
      pg.fill();
      pg.stroke();
      addText(panel, "ĐỊt MẸ MÀY JHVN MOD v2.2", 25, 300, 42, 0, panelHeight / 2 - 30, "center");
      ...
      addText(pageBasic, "⚡ Speed nhanh game bạn, server mới level chỉ dưới 40", 14, 420, 28, 0, y - 6, "center"); y -= gap;
  ...
  function showHiddenEvents(){
    ...
    try {
      ...
      if (moduleUtil) {
        ...
        moduleUtil._closeModuleId = moduleUtil._closeModuleId || {};
      }
      ...
      /* FORCE MO TAT CA MODULE MA ACTIVITY THAM CHIEU */
      for (var i = 0; i < activity.actData.length; i++) {
        var act = activity.actData[i];
        if (!act) continue;
        var mid = act.showContent && (act.showContent.moduleOpen || act.showContent.moduleId);
        if (mid && typeof mid === "object") mid = mid.id;
        if (mid && moduleUtil) {
          moduleUtil._hasOpen[mid] = mid;
          delete moduleUtil._closeModuleId[mid];
        }
        /* cung force cho showContent.moduleOpen */
        var mid2 = act.showContent && act.showContent.moduleOpen;
        if (mid2 && typeof mid2 === "object" && mid2.id && moduleUtil) {
          moduleUtil._hasOpen[mid2.id] = mid2.id;
          delete moduleUtil._closeModuleId[mid2.id];
        }
      }

      /* HOOK getActivityInfoList de bo verify check */
      try {
