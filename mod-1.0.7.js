...
(function () {
  ...
  function openHiddenEvent(){
    ...
    if (!activity || !Array.isArray(list) || typeof activity.getActStatusAndLastTime !== "function") return;
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var modelOpen = model && model.openActivity;
      var modelOpenText = typeof modelOpen === "function" ? String(modelOpen).replace(/\s/g, "") : "";
      if (modelOpenText && modelOpenText !== "function(){}") {
        modelOpen.call(model, type);
        opened = true;
        L("hidden event model open type=" + type);
      } else if (typeof activity.openActivity === "function") {
    ...
    function ensureMenu(){
      ...
      var clearDataButton = addButton(pageSupport, "XÓA", 104, 32, panelWidth / 2 - 68, y, function(){
        ...
        try {
          ...
          L("local storage cleared; requesting official logout");
          flush();
  ...
  function showHiddenEvents(){
    ...
    try {
      // 1. Patch SDKUtil.isVerifyApp() → false (Apple review bypass)
      var SDK = window.SDKUtil;
      if (SDK && typeof SDK.isVerifyApp === "function") {
        SDK.isVerifyApp = function(){ return false; };
        L("SDKUtil.isVerifyApp patched");
      }

      // 2. Xóa NotShowEntranceActivity (danh sách type bị ẩn)
      ...
      // 3. Xóa examMaskActivityIds
      var m = getModels();
      if (m && m.ActivityModel && m.ActivityModel.examMaskActivityIds) {
        m.ActivityModel.examMaskActivityIds = {};
        L("examMaskActivityIds cleared");
      }

      eventPatched = true;
    } catch(e){ L("showHiddenEvents err: "+e.message); }
  }
  ...
  function tick(){
    ...
    if (!okA) okA = hookAd();
    hookCheatCheckLog();
    if (!eventPatched) showHiddenEvents();
