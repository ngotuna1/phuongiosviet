/* jhvnios mod - v2.2. Dat file nay tai thu muc goc jhvnios.app/mod.js. */
(function () {
  "use strict";
  ...
  function autoMailReward(){
    if (!autoReady("mail", 30000)) return;
    var models = getModels(), email = models && models.EmailModel, list = email && email.mailList;
    if (!email || typeof email.reqOneKeyExtract !== "function" || !email.maxPage || !list) return;
  ...
  function autoDailyReward(){
    ...
    var models = getModels(), daily = models && models.DailyTaskModel, configs = window.DynamicConfigData && window.DynamicConfigData.t_TaskDaily;
    if (!daily || !configs || !daily.taskData || !daily.taskData.records) return;
    for (var id in daily.taskData.records) {
      var record = daily.taskData.records[id], cfg = configs[id];
      if (record && record.finish && record.finish[0] >= 1 && (!record.got || record.got[0] < 1) && (!cfg || cfg.ad != 1)) {
        L("auto daily reward=" + id);
        daily.reqGettReward(id, false, function(){});
        return;
      }
    }
    var boxes = window.DynamicConfigData.t_TaskDailyBox;
    if (!boxes || !daily.reward || typeof daily.getActiveRewardState !== "function") return;
    for (var key in boxes) if (daily.getActiveRewardState(boxes[key]) === 1) {
  ...
  function autoHangReward(){
    ...
    if (chapter.showOfflineHangReward && autoReady("offlineHang", 60000)) {
      ...
      rpc.send("Chapter_OffineReceiveHangReward", {}, function(){ chapter.showOfflineHangReward = false; });
      return;
    }
    if (!autoReady("hang", 300000) || typeof chapter.getTotalOnlineTime !== "function" || chapter.getTotalOnlineTime() < 300) return;
    L("auto hang reward");
    rpc.send("Chapter_ReceiveHangReward", {}, function(){ if (chapter.updateOnline) chapter.updateOnline(0, 0, 0); });
  }
  ...
  function autoRuneEquip(){
    ...
    if (model && typeof model.reqEquipOnekeyEquipRune === "function") {
      L("auto rune equip");
  ...
  function autoMonsterBattle(){
    ...
    if (!model || !model.monsterInvade || typeof model.getCanSkipOrAutoState !== "function" || !model.getCanSkipOrAutoState()) return;
    ...
    var eventInfo = model.getMonsterInvadeCfg().eventInfo || [], canAuto = window.DynamicConfigData.t_MonsterInvadeConst.canAuto || [], autoMap = {};
  ...
  function autoSignReward(){
    ...
    if (seven && seven.status === false && !seven.endState && seven.daySignOrigin < 7 && typeof seven.reqDaySign === "function") {
  ...
  function autoEggLottery(){
    ...
    for (var key in map) {
      ...
      if (model && typeof model.reqLottery === "function" && typeof model.getEggConfigs === "function") {
        model.auto = true;
        if ((!model.canLottery || model.canLottery()) && typeof model.drawReward === "function") model.drawReward();
      }
    }
  }
  ...
  function autoTick(){
    try {
      ...
      if (CFG.autoMail) autoMailReward();
      if (CFG.autoDaily) autoDailyReward();
      if (CFG.autoHang) autoHangReward();
      if (CFG.autoPetLevel) autoPetUpgrade();
      if (CFG.autoSkillLevel) autoSkillUpgrade();
      if (CFG.autoTreasureLevel) autoTreasureUpgrade();
      if (CFG.autoRune) autoRuneEquip();
      if (CFG.autoBattleSkill) setAutoBattleSkill(true);
      if (CFG.autoMonster) autoMonsterBattle();
      if (CFG.autoSign) autoSignReward();
      if (CFG.autoEgg) autoEggLottery();
      if (CFG.autoEventReward) autoEventRewards();
