...
(function () {
  ...
  /* ===== PATCH SDKUtil.playVideo qua module de VoucherActivityModel bat ===== */
  function hookSDKAd(){
    try {
      var SDK = getExport("SDKUtil", "SDKUtil");
      if (!SDK || typeof SDK.playVideo !== "function") { SDK = window.SDKUtil; if (!SDK) return false; }
      if (SDK.playVideo.__jm) return true;
      var origSDK = SDK.playVideo;
      SDK.playVideo = function(actType, adId, cb){
        if (!CFG.noAd) return origSDK.apply(this, arguments);
        L("SDKUtil.playVideo bypass actType=" + actType);
        if (typeof cb === "function") setTimeout(cb, 50);
      };
      SDK.playVideo.__jm = true;
      L("SDKUtil.playVideo HOOKED (voucher bypass)");
      return true;
    } catch(e){ L("hookSDKAd err: "+e.message); return false; }
  }

  /* ===== NHAN VOUCHER: chi patch hasBuyIngoreAdGift, de game tu gui useItem:true ===== */
  function hookIgnoreAdGift(){
    try {
      if (typeof hookIgnoreAdGift.__done !== "undefined") return true;
      var gm = getExport("GiftModel", "GiftModel");
      if (!gm) return false;
      if (typeof gm.hasBuyIngoreAdGift !== "function") return false;
      gm.hasBuyIngoreAdGift = function(){ return true; };
      hookIgnoreAdGift.__done = true;
      L("GiftModel.hasBuyIngoreAdGift -> true (voucher bypass)");
      flush();
      return true;
  ...
  var n=0, okS=false, okA=false, okE=false, okI=false, okG=false, okD=false, okJ=false;
  ...
  function tick(){
    ...
    if (n===1 || n===40 || (okS&&okA&&okE&&n%20===0)) { L("tick "+n+" subHp="+okS+" ad="+okA+" event="+okE+" ingoreAd="+okI+" grade="+okG+" devour="+okD+" sdkAd="+okJ); flush(); }
    if (n<1200 && (!okS || !okA || !okE || !okI || !okG || !okD || !okJ)) later(tick);
  }
  installMenu();
  later(tick);
  later(autoTick);
