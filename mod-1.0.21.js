...
(function () {
  ...
  var CFG = {
    ...
    // autoAchievement removed: game khong co RPC nhan thuong thanh tich
  ...
  /* ===== GIA DANG KY BO QC DE NHAN VOUCHER ===== */
  function hookIgnoreAdGift(){
    try {
      var models = getModels();
      if (!models) return false;
      if (models.__jm_iag) return true;
      var gm = models.GiftModel || getExport("GiftModel", "GiftModel");
      if (gm && typeof gm.hasBuyIngoreAdGift === "function") {
        gm.hasBuyIngoreAdGift = function(){ return true; };
        L("GiftModel.hasBuyIngoreAdGift patched -> true");
      }
      var vm = models.VoucherActivityModel;
      if (vm && typeof vm.reqVoucherTaskGetTaskReward === "function" && !vm.__jm_vad) {
        vm.__jm_vad = true;
        vm.reqVoucherTaskGetTaskReward = function(recordId){
          var RPC = getExport("RPCReq", "RPCReq");
          if (!RPC || typeof RPC.send !== "function") return;
          L("voucher claim recordId=" + recordId);
          /* thu Activity_VoucherTask_GetTaskReward voi useItem:true */
          RPC.send("Activity_VoucherTask_GetTaskReward", {
            recordId: recordId,
            useItem: true
          }, function(){
            L("voucher claim OK recordId=" + recordId);
            try {
              var d = getExport("Dispatcher","Dispatcher")||window.Dispatcher;
              if(d&&typeof d.dispatchEvent==="function") d.dispatchEvent("VoucherActivity_update");
            } catch(e2){}
          }, function(e){
            L("voucher claim ERR recordId=" + recordId + " code=" + (e?e.repError:"null") + " - fallback to Activity_GetTaskReward");
            /* fallback: thu dung Activity_GetTaskReward generic */
            if (RPC && typeof RPC.send === "function") {
              RPC.send("Activity_GetTaskReward", {
                activityType: 447,
                taskId: recordId
              }, function(){
                L("voucher claim fallback OK recordId=" + recordId);
                try {
                  var d = getExport("Dispatcher","Dispatcher")||window.Dispatcher;
                  if(d&&typeof d.dispatchEvent==="function") d.dispatchEvent("VoucherActivity_update");
                } catch(e2){}
              }, function(e2){
                L("voucher claim fallback ERR recordId=" + recordId + " code=" + (e2?e2.repError:"null"));
              });
            }
          });
        };
        L("VoucherActivityModel hooked");
      }
      models.__jm_iag = true;
      flush();
      return true;
    } catch(e){ L("hookIgnoreAdGift err: "+e.message); flush(); return false; }
  }

  /* ===== MO TOI DA GIOI HAN TRONG CON DUONG TU HANH ===== */
  function hookGradeMax(){
    try {
      ...
      if (typeof gm.checkCanLevelUp === "function") {
        var origCC = gm.checkCanLevelUp;
        gm.checkCanLevelUp = function(){
          var s = origCC.apply(this, arguments);
          if (s === 3) return 2;
          if (s === 0) return 1;
          return s;
        };
        L("GradeModel.checkCanLevelUp HOOKED (3->2, 0->1)");
      }
      if (typeof gm.getTaskRecordsData === "function" && !gm.getTaskRecordsData.__jm) {
        var origRec = gm.getTaskRecordsData;
        gm.getTaskRecordsData = function(recordId){
          var r = origRec.apply(this, arguments);
          if (r) return r;
          return { finish: [1], got: [0], acc: 0 };
        };
        gm.getTaskRecordsData.__jm = true;
        L("GradeModel.getTaskRecordsData HOOKED (fake finish if missing)");
      }
      gm.__jm_grade = true;
      L("GradeModel hooks installed (unlock grade + auto task)");
      flush();
      return true;
  ...
  function autoGrade(){
    ...
    var models = getModels(), grade = models && models.GradeModel, dynamic = window.DynamicConfigData;
    if (!grade || typeof grade.checkCanLevelUp !== "function" || !dynamic || !dynamic.t_Grade || !dynamic.t_GradeTask) return;
    var curLevel = grade.getCurLevel ? grade.getCurLevel() : 1;
    var gradeCfg = dynamic.t_Grade[curLevel];
    if (gradeCfg && gradeCfg.taskId) {
      for (var i = 0; i < gradeCfg.taskId.length; i++) {
        var task = dynamic.t_GradeTask[gradeCfg.taskId[i]];
        if (!task) continue;
        var record = grade.getTaskRecordsData ? grade.getTaskRecordsData(task.recordId) : null;
        var finished = record && record.finish && record.finish[0] >= 1;
        var got = record && record.got && record.got[0] >= 1;
        if (finished && !got && typeof grade.reqGettReward === "function") {
          L("auto grade claim reward recordId=" + task.recordId);
          grade.reqGettReward(task.recordId);
          return;
        }
      }
    }
    var state = grade.checkCanLevelUp ? grade.checkCanLevelUp() : 0;
    if (state >= 2 && typeof grade.reqLevelUp === "function") {
      L("auto grade level up (state=" + state + ")");
  ...
  /* ===== AUTO NHAN THUONG EVENT (DUNG CO CHE RECORD) ===== */
  ...
  function findEventReward(){
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var model = models.getActModelByActType && models.getActModelByActType(Number(act.type));
      if (!model) continue;
      /* doc records tu nhieu dang khac nhau */
      var records = null;
      if (model.taskData && model.taskData.records) records = model.taskData.records;
      else if (model.reward && typeof model.reward === "object") records = model.reward;
      else if (model._reward && typeof model._reward === "object") records = model._reward;
      else if (model.getRecord && typeof model.getRecord === "function") continue; /* can lay cu the */
      if (!records) continue;
  ...
  /* ===== MO EVENT AN ===== */
  var hiddenEventCandidates = [];
  var hiddenEventCandidateKeys = {};
  var hiddenEventOpenIndex = 0;

  function rememberHiddenEvent(act){
    var key = String(act.type) + ":" + String(act.id);
    if (hiddenEventCandidateKeys[key]) return;
    hiddenEventCandidateKeys[key] = true;
    hiddenEventCandidates.push(act);
  }

  function unlockHiddenEventEntries(){
    var models = getModels(), activity = models && models.ActivityModel, list = activity && activity.actData;
    var moduleUtil = getExport("ModuleUtil", "ModuleUtil");
    var activityMap = getExport("ActivityMap", "ActivityMap");
    if (!activity || !Array.isArray(list) || !moduleUtil) return null;

    var notShow = activityMap && activityMap.NotShowEntranceActivity || {};
    var examMask = activity.examMaskActivityIds || {};
    for (var i = 0; i < list.length; i++) {
      var act = list[i];
      if (!act) continue;
      var type = Number(act.type);
      var moduleId = act.showContent && (act.showContent.moduleOpen || act.showContent.moduleId);
      if (moduleId && typeof moduleId === "object") moduleId = moduleId.id;
      var wasHidden = !!act.cannotShow || !!notShow[type] || !!examMask[type];
      if (moduleId && typeof moduleUtil.moduleOpen === "function") {
        try { if (!moduleUtil.moduleOpen(moduleId, false)) wasHidden = true; } catch(e) {}
      }
      if (wasHidden) {
        rememberHiddenEvent(act);
        act.cannotShow = false;
        if (moduleId) {
          moduleUtil._hasOpen = moduleUtil._hasOpen || {};
          moduleUtil._hasOpen[moduleId] = moduleId;
          if (moduleUtil._closeModuleId) delete moduleUtil._closeModuleId[moduleId];
        }
      }
    }

    if (activityMap) activityMap.NotShowEntranceActivity = {};
    ...
    return { models: models, activity: activity, moduleUtil: moduleUtil };
  }

  function openHiddenEvent(){
    var state = unlockHiddenEventEntries();
    if (!state) {
      L("hidden event unavailable");
      flush();
      return;
    }
    if (!hiddenEventCandidates.length) {
      L("no hidden event in server actData");
      flush();
      return;
    }
    for (var i = 0; i < hiddenEventCandidates.length; i++) {
      var index = hiddenEventOpenIndex++ % hiddenEventCandidates.length;
      var act = hiddenEventCandidates[index];
      if (!act) continue;
      if (typeof state.activity.getActStatusAndLastTime === "function" && state.activity.getActStatusAndLastTime(act.id)[0] !== 2) continue;
      var type = Number(act.type);
      var moduleId = act.showContent && (act.showContent.moduleOpen || act.showContent.moduleId);
      if (moduleId && typeof moduleId === "object") moduleId = moduleId.id;
      if (moduleId && typeof state.moduleUtil.openModule === "function") {
        state.moduleUtil.openModule(moduleId, true);
        L("hidden event opened type=" + type + " id=" + act.id + " module=" + moduleId);
        flush();
        return;
      }
      var model = state.models.getActModelByActType && state.models.getActModelByActType(type);
      if (model && typeof model.openActivity === "function") {
        model.openActivity(type);
        L("hidden event model opened type=" + type + " id=" + act.id);
        flush();
        return;
      }
    }
    L("hidden event exists but is not active/openable");
    flush();
  }
  ...
  var n=0, okS=false, okA=false, okE=false, okI=false, okG=false, okD=false;
  ...
  function showHiddenEvents(){
    ...
    try {
      ...
      if (!unlockHiddenEventEntries()) return false;
      L("hidden event visibility rebuilt candidates=" + hiddenEventCandidates.length);
      eventPatched = true;
      flush();
      return true;
  ...
  function hookCheatCheckLog(){
    ...
    var origDo = target.doCheatCheck;
    target.doCheatCheck = function(){
      var self = this;
      var previous = Number(self.checkNowTime);
      var release = typeof window.__IS_RELEASE__ !== "undefined" ? window.__IS_RELEASE__ : "unknown";
      L("cheat check start prev="+previous+" isCheck="+self.isCheck+" release="+release);
      flush();

      var result = origDo.apply(this, arguments);
      try {
        if (typeof setTimeout === "function") setTimeout(function(){
          var current = Number(self.checkNowTime);
          if (current !== previous && isFinite(current)) {
            var delta = previous >= 0 && isFinite(previous) ? (current - previous) / 1000 : -1;
            L("cheat check response prev="+previous+" current="+current+" deltaSec="+delta+" thresholdSec=30 isCheck="+self.isCheck);
          } else {
            L("cheat check no new server time prev="+previous+" current="+current);
          }
          flush();
        }, 1500);
      } catch(e){ L("cheat check log err "+e); flush(); }
      return result;
    };

    if (typeof target.initCheatCheck === "function") {
      var origInit = target.initCheatCheck;
      target.initCheatCheck = function(){
        var self = this;
        L("cheat check init prev="+self.checkNowTime);
        var result = origInit.apply(this, arguments);
        try {
          if (typeof setTimeout === "function") setTimeout(function(){
            L("cheat check init response current="+self.checkNowTime);
            flush();
          }, 1500);
        } catch(e){ L("cheat init log err "+e); flush(); }
        return result;
      };
    }

    cheatCheckLogPatched = true;
    L("ServerTimeModel diagnostic installed");
    flush();
    return true;
  }
  ...
  function tick(){
    ...
    if (!okI) okI = hookIgnoreAdGift();
    if (!okG) okG = hookGradeMax();
    if (!okD) okD = hookDevourOpen();
    // applySpeed removed - setTimeScale bị game detect
    if (n===1 || n===40 || (okS&&okA&&okE&&n%20===0)) { L("tick "+n+" subHp="+okS+" ad="+okA+" event="+okE+" ingoreAd="+okI+" grade="+okG+" devour="+okD); flush(); }
    if (n<1200 && (!okS || !okA || !okE || !okI || !okG || !okD)) later(tick);
  }
  installMenu();
  later(tick);
  later(autoTick);
