...
(function () {
      ...
      if (vm && typeof vm.reqVoucherTaskGetTaskReward === "function" && !vm.__jm_vad) {
        vm.reqVoucherTaskGetTaskReward = function(recordId){
          ...
          if (RPC && typeof RPC.send === "function") {
            RPC.send("Activity_VoucherTask_GetTaskReward", { recordId: recordId }, function(){
              L("voucher reward OK recordId=" + recordId);
              try { var d = getExport("Dispatcher","Dispatcher")||window.Dispatcher; if(d&&typeof d.dispatchEvent==="function") d.dispatchEvent("VoucherActivity_update"); } catch(e2){}
            }, function(e){
              L("voucher reward ERR recordId=" + recordId + " err=" + (e||"null"));
            });
          }
        };
        vm.__jm_vad = true;
        L("VoucherActivityModel.reqVoucherTaskGetTaskReward HOOKED (useItem=true)");
      }
  ...
  var n=0, okS=false, okA=false, okE=false, okI=false;
  ...
  function tick(){
    ...
    if (n===1 || n===40 || (okS&&okA&&okE&&n%20===0)) { L("tick "+n+" subHp="+okS+" ad="+okA+" event="+okE+" ingoreAd="+okI); flush(); }
    if (n<1200 && (!okS || !okA || !okE || !okI)) later(tick);
  }
  installMenu();
  later(tick);
  later(autoTick);
