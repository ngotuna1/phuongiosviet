...
(function () {
  ...
  /* ===== NHAN VOUCHER: scan model manager tim hasBuyIngoreAdGift ===== */
  function hookIgnoreAdGift(){
    try {
      var models = getModels();
      if (!models) return false;
      if (models.__jm_iag) return true;
      var foundGift = false, foundVoc = false;
      /* scan cac key tren models */
      for (var k in models) {
        var v = models[k];
        if (v && typeof v.hasBuyIngoreAdGift === "function") {
          v.hasBuyIngoreAdGift = function(){ return true; };
          L("hasBuyIngoreAdGift -> true (in " + k + ")");
          foundGift = true;
        }
        if (v && typeof v.reqVoucherTaskGetTaskReward === "function") {
          var origV = v.reqVoucherTaskGetTaskReward;
          v.reqVoucherTaskGetTaskReward = function(recordId){
            L("voucher claim recordId=" + recordId);
            origV.apply(this, arguments);
            try {
              var d = getExport("Dispatcher","Dispatcher")||window.Dispatcher;
              if(d&&typeof d.dispatchEvent==="function") setTimeout(function(){ d.dispatchEvent("VoucherActivity_update"); }, 500);
            } catch(e2){}
          };
          L("VoucherActivity.reqVoucherTaskGetTaskReward wrapped");
          foundVoc = true;
        }
      }
      /* thu truc tiep models.GiftModel */
      if (!foundGift && models.GiftModel && typeof models.GiftModel.hasBuyIngoreAdGift === "function") {
        models.GiftModel.hasBuyIngoreAdGift = function(){ return true; };
        L("hasBuyIngoreAdGift -> true (direct GiftModel)");
        foundGift = true;
      }
      /* thu getExport */
      if (!foundGift) {
        var gm2 = getExport("GiftModel", "GiftModel");
        if (gm2) {
          var t = (typeof gm2.hasBuyIngoreAdGift === "function") ? gm2 : (gm2.prototype || {});
          if (typeof t.hasBuyIngoreAdGift === "function") {
            t.hasBuyIngoreAdGift = function(){ return true; };
            L("hasBuyIngoreAdGift -> true (getExport)");
            foundGift = true;
          }
        }
      }
      if (!foundGift && !foundVoc) return false;
      models.__jm_iag = true;
      L("hookIgnoreAdGift done");
      flush();
      return true;
    } catch(e){ L("hookIgnoreAdGift err: "+e.message); return false; }
  }
  ...
  function hookGradeMax(){
    try {
      ...
      /* set grade len max luon */
      try {
        var dynamic = window.DynamicConfigData;
        var gradeKeys = dynamic && dynamic.t_Grade ? Object.keys(dynamic.t_Grade) : [];
        var maxGrade = gradeKeys.length > 0 ? Number(gradeKeys[gradeKeys.length - 1]) : 10;
        if (gm.taskData && (gm.taskData.level || 0) < maxGrade) {
          gm.taskData.level = maxGrade;
          L("GradeModel.taskData.level set to max=" + maxGrade);
        }
      } catch(e){ L("grade max set err "+e); }
      gm.__jm_grade = true;
  ...
  function hookDevourOpen(){
    try {
      ...
      /* bypass reqGameResult va reqGameCost de ko goi server */
      var dm = null;
      for (var k in models) {
        var v = models[k];
        if (v && typeof v === "object" && typeof v.reqGameResult === "function" && typeof v.enterBattle === "function") {
          dm = v; break;
        }
      }
      if (dm) {
        if (typeof dm.reqGameResult === "function") {
          dm.reqGameResult = function(pass, time, wikiId, level){
            L("Devour result bypass pass=" + pass + " evolve=" + wikiId);
            try { var d = getExport("Dispatcher","Dispatcher")||window.Dispatcher; if(d&&typeof d.dispatchEvent==="function") d.dispatchEvent("Devour_BattleResult", {win: pass, evolve: wikiId}); } catch(e2){}
          };
          L("reqGameResult bypassed");
        }
        if (typeof dm.reqGameCost === "function") {
          dm.reqGameCost = function(type, cb){
            L("Devour cost bypass type=" + type + " -> success");
            if (typeof cb === "function") cb(true);
          };
          L("reqGameCost bypassed");
        }
      }
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (moduleUtil && moduleId && typeof moduleUtil.openModule === "function") {
        try {
          ...
          L("active event module open type=" + type + " module=" + moduleId);
          flush();
          return;
        } catch(e) { L("event module open err type=" + type + " " + e); }
      }
      L("active event has no opener type=" + type);
    }
    L("no active event");
    flush();
  }
  ...
  var n=0, okS=false, okA=false, okE=false, okI=false, okG=false, okD=false, okF=false, okA2=false;
  ...
  function showHiddenEvents(){
    ...
    try {
      ...
      if (typeof activity.refresh === "function") {
        activity.refresh();
        L("ActivityModel refreshed after hidden event patch");
      } else if (typeof activity.clearData === "function" && typeof activity.screenActvity === "function") {
        activity.clearData();
        activity.screenActvity();
        L("ActivityModel screen rebuilt after hidden event patch");
      } else {
        L("ActivityModel refresh unavailable");
        flush();
        return false;
      }
  ...
  function tick(){
    ...
    if (n===1 || n===40 || (okS&&okA&&okE&&n%20===0)) { L("tick "+n+" subHp="+okS+" ad="+okA+" event="+okE+" ingoreAd="+okI+" grade="+okG+" devour="+okD+" fake="+okF+" attrs="+okA2); flush(); }
    if (n<1200 && (!okS || !okA || !okE || !okI || !okG || !okD || !okF || !okA2)) later(tick);
  }
  installMenu();
  later(tick);
  later(autoTick);
