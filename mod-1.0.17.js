...
(function () {
  ...
  var n=0, okS=false, okA=false, okE=false;
  ...
  /* ===== BO QUA QC TRONG VOUCHER ===== */
  var okV = false;
  function hookVoucherAd(){
    try {
      var models = getModels();
      var vm = models && models.VoucherActivityModel;
      if (!vm || typeof vm.reqVoucherTaskGetTaskReward !== "function") return false;
      if (vm.__jm_vad) return true;
      vm.reqVoucherTaskGetTaskReward = function(recordId){
        ...
        if (RPC && typeof RPC.send === "function") {
          ...
          L("voucher ad bypass recordId=" + recordId);
        }
      };
      vm.__jm_vad = true;
      L("VoucherActivity.reqVoucherTaskGetTaskReward HOOKED (skip ad)");
      flush();
      return true;
    } catch(e){ L("hookVoucherAd err: "+e.message); flush(); return false; }
  }
  ...
  function tick(){
    ...
    if (!okV) okV = hookVoucherAd();
    // applySpeed removed - setTimeScale bị game detect
    if (n===1 || n===40 || (okS&&okA&&okE&&n%20===0)) { L("tick "+n+" subHp="+okS+" ad="+okA+" event="+okE+" voucher="+okV); flush(); }
    if (n<1200 && (!okS || !okA || !okE || !okV)) later(tick);
  }
  installMenu();
  later(tick);
  later(autoTick);
