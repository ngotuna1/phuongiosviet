...
(function () {
  ...
  function hookGradeMax(){
    try {
      ...
      /* set grade + player level len max */
      try {
        ...
        /* set player level + exp cao de mo khoa canh gioi */
        var pm = models && models.PlayerModel;
        if (pm) {
          if (pm.level != null && pm.level < 500) {
            pm.level = 500;
            L("PlayerModel.level set to 500");
          }
